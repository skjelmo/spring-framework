...
/**
 * Tests for {@link DefaultRestTemplateObservationConvention}.
 *
 * @author Brian Clozel
 */
class DefaultRestTemplateObservationConventionTests {

	private final DefaultRestTemplateObservationConvention observationConvention = new DefaultRestTemplateObservationConvention();

	@Test
	void supportsOnlyClientHttpObservationContext() {
		assertThat(this.observationConvention.supportsContext(new RestTemplateObservationContext())).isTrue();
		assertThat(this.observationConvention.supportsContext(new Observation.Context())).isFalse();
	}

	@Test
	void addsKeyValuesForNullExchange() {
		RestTemplateObservationContext context = new RestTemplateObservationContext();
	...
	void addsKeyValuesForRequestWithUriTemplate() {
		RestTemplateObservationContext context = createContext(
	...
	private RestTemplateObservationContext createContext(ClientHttpRequest request, ClientHttpResponse response) {
		RestTemplateObservationContext context = new RestTemplateObservationContext();
